...
int vk_init() {
    ...
    VkResult result = vkEnumeratePhysicalDevices(g_instance, &device_count, NULL);
    if (result != VK_SUCCESS) {
        printf("❌ Failed to enumerate Vulkan devices\n");
    ...
    if (!devices) {
        printf("❌ Failed to allocate device array\n");
        vkDestroyInstance(g_instance, NULL);
        g_instance = VK_NULL_HANDLE;
        return 0;
    }
    result = vkEnumeratePhysicalDevices(g_instance, &device_count, devices);
    if (result != VK_SUCCESS) {
        printf("❌ Failed to enumerate Vulkan device list\n");
        free(devices);
        vkDestroyInstance(g_instance, NULL);
        g_instance = VK_NULL_HANDLE;
        return 0;
    ...
    for (uint32_t i = 0; i < device_count; i++) {
        ...
        if (!queue_props) {
            printf("❌ Failed to allocate queue family property array\n");
            free(devices);
            vkDestroyInstance(g_instance, NULL);
            g_instance = VK_NULL_HANDLE;
            return 0;
...
void* vk_allocate_buffer(size_t size_bytes, int device_local) {
    ...
    if (vkBindBufferMemory(g_device, buf->buffer, buf->memory, 0) != VK_SUCCESS) {
    ...
    if (!device_local) {
        if (vkMapMemory(g_device, buf->memory, 0, size_bytes, 0, &buf->mapped_ptr) != VK_SUCCESS) {
            printf("❌ Failed to map host-visible buffer memory\n");
            vkDestroyBuffer(g_device, buf->buffer, NULL);
            vkFreeMemory(g_device, buf->memory, NULL);
            free(buf);
            return NULL;
...
float vk_dispatch_compute(void* shader, void* input_buf, void* weights_buf, void* output_buf,
    ...
    if (!shader || !g_device || !input_buf || !weights_buf || !output_buf) return 0.0f;
    ...
    if (result != VK_SUCCESS) {
        printf("❌ Failed to create fence for dispatch\n");
        vkFreeCommandBuffers(g_device, g_command_pool, 1, &cmd_buffer);
        return 0.0f;
    }
    ...
    // Wait for completion
    if (vkWaitForFences(g_device, 1, &fence, VK_TRUE, UINT64_MAX) != VK_SUCCESS) {
...
float vk_dispatch_conv2d(void* shader, void* input_buf, void* weights_buf, void* output_buf,
    ...
    if (!shader || !input_buf || !weights_buf || !output_buf) {
        return -1.0f;
    }
...
float vk_dispatch_conv2d_timed(void* shader, void* input_buf, void* weights_buf, void* output_buf,
    ...
    result = vkCreateFence(g_device, &fence_info, NULL, &fence);
    if (result != VK_SUCCESS) {
        printf("❌ Failed to create fence for timed dispatch\n");
        vkFreeCommandBuffers(g_device, g_command_pool, 1, &cmd_buffer);
        return -1.0f;
    }
